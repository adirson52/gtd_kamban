...
module.exports = async (req, res) => {
  ...
  const token = process.env.GITHUB_TOKEN; // ⚠️ Configure isso na Vercel
  ...
  try {
    ...
    const content = Buffer.from(fileData.content, "base64").toString("utf-8").trim();
    const linhas = content.split("\n");
    const lastLine = linhas[linhas.length - 1];
    const lastId = parseInt(lastLine.split(",")[0] || "0", 10);
